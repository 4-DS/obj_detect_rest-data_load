...
Rest CV Pipeline представляет собой сценарий, при котором требуется передать модель, обученную в фреймворке PyTorch, в виде образа docker с REST API. В данном примере модель YOLOX_S из mmdetection дообучается на части валидационной выборки COCO датасета.
...
<div align="center">
  ...
  <div>&nbsp;</div>
  <div align="center">
    <b><font size="3">Структура Rest CV-Pipeline</font></b>
  </div>
  <div>&nbsp;</div>
</div>

# Описание step Rest CV-Pipeline
## Data_load
### Логика компонента:
...
1. Конвертация модели
   - Модель, обученная на предыдущем этапе CV-Pipeline Model_Train, конвертируется в формат, соответствующий определенным сценариям. Например, если выбран сценарий REST CV-Pipeline, модель может быть конвертирована в формат ONNX, который обеспечивает возможность развертывания модели в виде REST-сервиса.
